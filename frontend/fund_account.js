...
async function main () {
  ...
  if (!sudoKey) {
    ...
    console.error("   Please run: export SUDO_KEY='your_private_seed' && node fund_account.js");
    process.exit(1);
  }

  let alice;
  try {
    alice = keyring.addFromUri(sudoKey);
  } catch (error) {
    console.error("❌ Error: Invalid SUDO_KEY format.");
    process.exit(1);
  }
